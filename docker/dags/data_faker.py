...
def generate_products(dir, filename, num_products, use_s3, fs):
    filename = get_true_cwd(use_s3) + filename
    mode = append_or_write(filename)
    product_ids = []
...
def generate_transactions(dir, filename, num_transactions, user_ids, product_ids, use_s3, fs):
    filename = get_true_cwd(use_s3) + filename
    mode = append_or_write(filename)
    ...
    with fs.open(filename, mode, newline='') if use_s3 else open(filename, mode, newline='') as csvfile:
        ...
        statuses = ["Ordered", "Processing", "Declined", "Canceled", "Roast Complete", "Shipped", "Complete"]
        for _ in range(num_transactions):
            writer.writerow([generate_id(), random.choice(user_ids), random.choice(product_ids), random.randint(1,5), random.choice(statuses), fake.date_between_dates(date_start=TODAY - timedelta(days=30), date_end=TODAY)])
...
def generate_data(user_path, retail_products_path, transactions_path, target=None):
    ...
    product_ids = generate_products(retail_products_path, NUM_PRODUCTS,  use_s3, fs=s3 )
    generate_transactions(transactions_path, NUM_TRANSACTIONS, user_ids, product_ids,  use_s3, fs=s3 )
